...
//    NSString* tmp, *org = message, *appended = @"";
//    while (true)
//    {
//        tmp = message.length > 100 ? [message substringToIndex:100] : message;
//
//        appended = [appended stringByAppendingString:tmp];
//
//        data = [tmp dataUsingEncoding:NSUTF8StringEncoding];
//        [_outputStream write:[data bytes] maxLength:[data length]];
//        if(message.length <= 100) break;
//        message = [message substringFromIndex:100];
//    }
//
//    if ([org isEqualToString:appended]) {
//        NSLog(@"MATCH: OK");
//    }else NSLog(@"MATCH: ERROR");
...
#warning UPDATE NEW NETSERVICE HERE
